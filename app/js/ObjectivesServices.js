...
angular.module('ObjectivesServices', ['ParseServices', 'DataServices'])
    ...
    .factory('LoadObjectivesForIncident', ['ParseQuery', 'ConvertParseObject', 'DataStore', 'CreateNewObjectives',
        function (ParseQuery, ConvertParseObject, DataStore, CreateNewObjectives) {
        return function ($scope, incident) {
           ...
           queryObjectives.equalTo("incident", incident);
        	ParseQuery(queryObjectives, {functionToCall:'first'}).then(function(objectivesObject){
        		if (objectivesObject){
